...
class FeatureCardIconWidget extends StatelessWidget {
  ...
  Widget _buildContent(
    ...
    if (!hasTitle) {
      // Icon only - center it
      return Center(
        child: _buildIconContainer(iconSize, minDimension),
      );
    }
    ...
    if (needsCompactLayout) {
      ...
      return Row(
        ...
        children: [
          Flexible(
            flex: 2,
            child: _buildIconContainer(iconSize * 0.8, minDimension),
          ),
    ...
    return Column(
      mainAxisAlignment: MainAxisAlignment.center,
      children: [
        _buildIconContainer(iconSize, minDimension),
        SizedBox(height: minDimension * 0.08),
        _buildTitleText(context, textSize),
      ],
    );
  }

  Widget _buildIconContainer(double iconSize, double minDimension) {
    return Container(
